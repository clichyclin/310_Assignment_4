...
This project explores the historical population of horses in Canada between 1906
and 1972 for each province.

## Data

Horse population data were sourced from the [Government of Canada's Open Data website](http://open.canada.ca/en/open-data) (Government of Canada, 2017a and 
Government of Canada, 2017b).

Sample of data for analysis shown in Table \@ref(tab:load-data).

```{r load-data}
horse_pop_data <- read_csv("00030067-eng.csv") |>
    select(Ref_Date, Value) |>
    slice(1:40)
knitr::kable(horse_pop_data,
             caption = "Horse Population between 1906 and 1972 in different 
             parts of Canada.")
...
``` {r scatter-plot}
ggplot(horse_pop_data, aes(x = Ref_Date, y = Value)) +
  geom_point()
```
...
scatter_horse_pops <- ggplot(horse_pop_data, 
                             aes(x = Ref_Date, y = Value)) +
                      geom_point() +
                      labs(x = "Number of Horses", y = "Year")

ggsave("../../results/scatter_horse_pops.png")
```

```{r display-scatter-plot, out.width="25%}
knitr::include_graphics("../../results/scatter_horse_pops.png")
```

Figure \@ref(fig:display-scatter-plot) : Horse populations for all provinces in Canada from 1906 - 1972
...
```{r display-scatter-plot, out.width = 55%}
